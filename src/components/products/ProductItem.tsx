...
const ProductItem = ({ id, name, unitPrice, imageURL, currentStock }: ProductListInterface) => {
    return (
        <Link href={`/products?productName=${name}&id=${id}`}>
            <section className="flex flex-col shadow-[rgba(50,_50,_105,_0.15)_0px_2px_5px_0px,_rgba(0,_0,_0,_0.05)_0px_1px_1px_0px] rounded-2xl hover:-translate-y-2 transition-transform">
                <div className="relative">
                    ...
                    { currentStock == 0 && <span className="absolute bottom-0 left-0 p-2 bg-red-200 uppercase text text-[0.8rem] font-medium tracking-wider">out of stock</span>}
                </div>
                <div className="p-4">
